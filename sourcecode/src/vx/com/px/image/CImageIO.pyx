...
cdef class CImageIO:
    ...
    def write_npz(self, outputdir, imgoname):
        height = self._img.height()
        width = self._img.width()

        superpixels = [[] for i in range(((self._img)._img.size_labels)) ]
        ...
        np.savez_compressed(outputdir+imgoname, width=width, height=height, superpixels=an_array)
        del an_array
